...
The subcommand (``build``, ``initramfs``, ``tools``, …) is extracted from
``sys.argv`` *before* parsing so that flags work in any position::

    ./build.py --arch=arm64 initramfs      # works
    ./build.py initramfs --arch=arm64      # also works
    ARCH=arm64 ./build.py initramfs        # also works
"""
